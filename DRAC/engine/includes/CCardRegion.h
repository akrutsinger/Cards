...
//Drag Modes
const unsigned int CRD_DRAGSTACKS		= (1 << 0); //Drag complete stacks
const unsigned int CRD_DRAGCARDS		= (1 << 1); //Drag individual cards
const unsigned int CRD_DRAGTOP			= (1 << 2); //Drag only top of stack
const unsigned int CRD_DRAGFACEUP		= (1 << 3); //Variation of 1, but only cards facing up
const unsigned int CRD_DRAGOPPOSITECOLORS		= (1 << 4); //Drag card iff all cards on top of it are opposite colors. Cards are assumed face up
const unsigned int CRD_DRAGHIGHERBY1	= (1 << 5); //Drag stack if the stacks top card is one 1 higher than the card below it
const unsigned int CRD_DRAGLOWERBY1		= (1 << 6); //Drag stack if the stacks top most card is 1 lower than the card below
const unsigned int CRD_DRAGNONE			= (1 << 15);	//Disables dragging
...
const unsigned int CRD_DOWRAP		= (1 << 11); // allows Ace and Kings to play off each other
//On empty stacks, used in most solitaire games
const unsigned int CRD_DOACE		= ( 1 << 12 ); // TRUE:accept only aces					FALSE:don't accept ace
const unsigned int CRD_DOKING		= ( 1 << 13 ); // TRUE:accept only kings				FALSE:don't accept king
...
class CCardRegion
{
	public:
		...
		void  GetOffsets(int &xoff, int &yoff)			{ xoff = xOffset; yoff = yOffset; }
