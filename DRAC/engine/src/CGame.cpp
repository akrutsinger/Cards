...
bool CGame::InitDrag(CCardStack *CargoStack, int x, int y)
{
	...
	if(SourceRegion->GetDragMode()&CRD_DRAGNONE){
		return false;
	}

		if(SourceRegion->GetDragMode()&CRD_DRAGTOP){
			if(!(SourceRegion->Size() - 1 == idx)){
					return false;
			}
		}

		if(SourceRegion->GetDragMode()&CRD_DRAGFACEUP){
			if(!(SourceRegion->CardFaceUp(idx))){
					return false;
			}
		}

		if(SourceRegion->GetDragMode()&CRD_DRAGOPPOSITECOLORS){
			if(SourceRegion->CardFaceUp(idx)){
				for(int i = idx; i < SourceRegion->Size() - 1; i++){
					if((SourceRegion->GetCard(i).IsBlack() && (SourceRegion)->GetCard(i + 1).IsBlack()) ||
						(SourceRegion->GetCard(i).IsRed() && (SourceRegion)->GetCard(i + 1).IsRed())){
						return false;
					}
				}
			}else
		...
		if(SourceRegion->GetDragMode()&CRD_DRAGHIGHERBY1){
			if(SourceRegion->CardFaceUp(idx)){
				for(int i = idx; i < SourceRegion->Size() - 1; i++){
					if((SourceRegion->GetCard(i).Rank() + 1) != (SourceRegion)->GetCard(i + 1).Rank()){
						return false;
					}
				}
			}else
		...
		if(SourceRegion->GetDragMode()&CRD_DRAGLOWERBY1){
			if(SourceRegion->CardFaceUp(idx)){
				for(int i = idx; i < SourceRegion->Size() - 1; i++){
					if((SourceRegion->GetCard(i).Rank() - 1) != (SourceRegion)->GetCard(i + 1).Rank()){
						return false;
		...
		//We always push the SourceRegion to the DragStack. If we don't need to we return false and never get here anyway
